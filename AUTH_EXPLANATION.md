...
> **⚠️ 注意**: このドキュメントは古い実装の説明を含んでいます。  
> **最新の仕様については [`AUTH_SPECIFICATION.md`](./AUTH_SPECIFICATION.md) を参照してください。**
...
**重要な変更点**:
- `state`パラメータは**データベース**に保存されます（セッションクッキーに依存しない）
- トークンは**AES-256-CBC**で暗号化してデータベースに保存されます
